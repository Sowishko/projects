...
namespace _2._7тп
{
    public partial class Form1 : Form
    {
        ...
        {
            if (enterprise.kolWorkers - 2 < 0)
            {
                MessageBox.Show("Нужно больше деталей", "Сообщение");
                return;
            }
            enterprise.kolWorkers -= 2;
            enterprise.ws[0].ObrM(2);
            Wst.Text = enterprise.kolWorkers.ToString();
        }

        private void IzgMet_Click(object sender, EventArgs e)
        {
            if (enterprise.kolWorkers - 10 < 0)
            {
                MessageBox.Show("Нужно больше деталей", "Сообщение");
                return;
            }
            enterprise.kolWorkers -= 10;
            enterprise.ws[1].CreateTruba(10);
            Wst.Text = enterprise.kolWorkers.ToString();
        }
    }
    ...
    class enterprise
    {
        ...
        {
            kolWorkers += ws;
            Notify?.Invoke(" Изготовлены детали");
        }
    }
    ...
    class WS : enterprise,ICreateM, IObr
    {
        ...
        public void CreateTruba(int c)
        {
            for (int i = 0; i < c; i++)
            {
                ...
                workers.Last().Name = "Деталь" + counter;
                counter++;
                Notify?.Invoke(" Механизм изготовлен");
            }
